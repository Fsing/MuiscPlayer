...
std::string LoginController::myLogin(std::string username, std::string password)
{
    Json::Value root;
...
//获得指定表最大id
int LoginController::getMaxid(string tableName)
{
    MYSQL mysql;
    mysql_init(&mysql);
    if(!mysql_real_connect(&mysql,"localhost","fsing","fsing","Fsing",3306,nullptr,0)){
...
bool LoginController::insertUser(string username,string userpassword){
    ...
    std::sprintf(sql,"insert into Account(name,password,label,sex,birthday,address,icon)"
                     " values('%s','%s','label','sex',sysdate(),'china','userIcon.png')",
...
bool LoginController::hasTable(std::string tableName)
{
    MYSQL mysql;
    mysql_init(&mysql);
    if(!mysql_real_connect(&mysql,"localhost","fsing","fsing","Fsing",3306,nullptr,0)){
...
void LoginController::createCollectionRelationTable()
{
    MYSQL mysql;
    mysql_init(&mysql);
    if(!mysql_real_connect(&mysql,"localhost","fsing","fsing","Fsing",3306,nullptr,0)){
...
bool LoginController::createAccountTable()
{
    MYSQL mysql;
    mysql_init(&mysql);
    if(!mysql_real_connect(&mysql,"localhost","fsing","fsing","Fsing",3306,nullptr,0)){
