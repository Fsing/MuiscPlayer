...
void LoginController::createSongList(Json::Value resultRoot)
{
    ...
    if(ret == "SUCCESS"){
        ...
        list.append("");
        list.append("");
        createSongListNameAndId.append(QString::fromStdString(resultRoot["songListName"].asString()));
        createSongListNameAndId.append(QString::fromStdString(resultRoot["songListID"].asString()));
...
QList<QString> LoginController::getCreateSongNameLists()
{
    ...
    std::cout << "CreateSongListNames:" << listNames.count() << std::endl;
    for (int i = 0; i < listNames.count(); i++){
        createSongNames.append(listNames[i]);//songlist name
        createSongNames.append(values[i][0]);//songlist Id
        std::cout << "********************************************" << std::endl;
        qDebug() <<values[i][0];
...
QList<QString> LoginController::getCollectSongListNames()
{
    auto collectSongLists = m_fan.collectedSongLists();
    auto listNames = collectSongLists.keys();
    auto values = collectSongLists.values();
    QList<QString> collectSongNames;
    std::cout << "CrollectSongListNames:" << listNames.count() << std::endl;
    for (int i = 0; i < listNames.count(); i++){
        collectSongNames.append(listNames[i]);//songlist name
        collectSongNames.append(values[i][0]);//songlist Id
        std::cout << "********************************************" << std::endl;
        qDebug() <<values[i][0];
        qDebug() << collectSongNames[i];
    }
    qDebug() << collectSongNames.count();

    return collectSongNames;
}

QList<QString> LoginController::getUserSongListNames()
{
    auto createSongLists = m_fan.createdSongLists();
    auto listNames1 = createSongLists.keys();
    auto values1 = createSongLists.values();
    QList<QString> songListNames;
    for (int i = 0; i < listNames1.count(); i++){
        songListNames.append(listNames1[i]);//songlist name
        songListNames.append(values1[i][0]);//songlist Id
    }
    ...
    auto values = collectSongLists.values();
    for (int i = 0; i < listNames.count(); i++){
        songListNames.append(listNames[i]);//songlist name
        songListNames.append(values[i][0]);//songlist Id
    }
    return songListNames;
}

QList<QString> LoginController::getAddSongListResult()
{
    return createSongListNameAndId;
}
