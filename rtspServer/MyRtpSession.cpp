...
void CRtpSession::thread_proc(long user_info)
{
    ...
    RTPIPv4Address addr(ntohl(inet_addr(m_client_ip)), 8000);
    ...
    while(IsDestroyed() == false){
        if(m_is_Play == true){
            if((bufLength = m_decodeSrc.getNextPacket(buffer,SEND_BUFFE_SIZE,pts)) > 0 ){
                ...
                //                    cout <<"Send packet  " << bufLength << "byt es" << endl;
...
//        if(has_send_length >= 4608){
//            RTPTime::Wait(RTPTime(0.8,0));        //等待1秒，再发送
//            has_send_length = 0;
//        }
        Sleep( 2000 * BUFF_MAX_SIZE/readBytesPerSec );
//                RTPTime::Wait(RTPTime(0.5,0));        //等待1秒，再发送
    }
